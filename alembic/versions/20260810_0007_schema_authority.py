...
Renumbered from 0006 during the wave-3 merge: session 03C's
20260810_0006_draft_preseason_and_adp_ids landed first and claimed that
revision id with the same parent, which would have left Alembic with a
duplicate revision and refused to run.
"""
...
_FEATURE_MATRIX_COLUMNS = (
    ...
    # prior_snap_share is session 02's pregame-knowable replacement for the
    # leaked snap_pct_off: last completed game's snap fraction, not the target
    # game's. It reached FeatureRow and FEATURE_COLS without ever reaching the
    # schema — the exact C-12 shape, a column owned by runtime code rather than
    # by a migration.
    ("prior_snap_share", "FLOAT"),
